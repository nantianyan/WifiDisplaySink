...
enum wfd_event_type {
    WFD_NOP               = 0, // interface test message
    WFD_ERROR            = 100,
    WFD_INFO              = 200,
};
...
enum wfd_info_type {
    ...
    WFD_INFO_UNKNOWN = 1,
    // 7xx
    WFD_INFO_RTSP_TEARDOWN = 700,
};

class WfdSinkListener: virtual public RefBase
{
public:
...
struct WifiDisplaySink : public AHandler {
    ...
    status_t setListener(const sp<WfdSinkListener>& listener);
...
private:
    ...
    sp<ALooper> mWfdSinkLooper;
    sp<WfdSinkListener> mListener;
    State mState;
    uint32_t mFlags;
