...
void DirectRenderer::DecoderContext::scheduleDecoderNotification() {
    ...
    sp<AMessage> notify =
        PlantUtils::newAMessage(kWhatDecoderNotify, this);
...
void DirectRenderer::AudioRenderer::schedulePushIfNecessary() {
    ...
    // has elapsed.
    (PlantUtils::newAMessage(kWhatPushAudio, this))->post(delayUs / 2);
}
...
void DirectRenderer::setFormat(size_t trackIndex, const sp<AMessage> &format) {
    sp<AMessage> msg = PlantUtils::newAMessage(kWhatSetFormat, this);
...
void DirectRenderer::internalSetFormat(
    ...
    sp<AMessage> notify = PlantUtils::newAMessage(kWhatDecoderNotify, this);
...
void DirectRenderer::queueAccessUnit(
    ...
    sp<AMessage> msg = PlantUtils::newAMessage(kWhatQueueAccessUnit, this);
    msg->setSize("trackIndex", trackIndex);
...
void DirectRenderer::scheduleVideoRenderIfNecessary() {
    ...
    (PlantUtils::newAMessage(kWhatRenderVideo, this))->post(delayUs);
}
...
void DirectRenderer::onRenderVideo() {
    ...
    while (!mVideoOutputBuffers.empty()) {
        ...
        if (info.mTimeUs + 15000ll < nowUs) {
            ++mNumFramesLate;
            ALOGD("DirectRenderer::onRenderVideo, late frames(>15ms) vs total frames (%d : %d)", mNumFramesLate, mNumFrames);
            // TODO: skip render late frame.
        }
        ++mNumFrames;
