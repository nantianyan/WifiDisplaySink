...
void TimeSyncer::startServer(unsigned localPort) {
    sp<AMessage> msg = PlantUtils::newAMessage(kWhatStartServer, this);
    msg->setInt32("localPort", localPort);
    msg->post();
}

void TimeSyncer::startClient(const char *remoteHost, unsigned remotePort) {
    sp<AMessage> msg = PlantUtils::newAMessage(kWhatStartClient, this);
...
void TimeSyncer::onMessageReceived(const sp<AMessage> &msg) {
    switch (msg->what()) {
        case kWhatStartClient:
        {
            ...
            sp<AMessage> notify = PlantUtils::newAMessage(kWhatUDPNotify, this);
...
void TimeSyncer::postSendPacket(int64_t delayUs) {
    (PlantUtils::newAMessage(kWhatSendPacket, this))->post(delayUs);
}

void TimeSyncer::postTimeout() {
    sp<AMessage> msg = PlantUtils::newAMessage(kWhatTimedOut, this);
