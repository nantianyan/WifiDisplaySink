...
ANetworkSession::Session::Session(
    ...
    if (mState == CONNECTED) {
        ...
        AString localAddrString = PlantUtils::newStringPrintf(
                "%d.%d.%d.%d",
                (addr >> 24),
                (addr >> 16) & 0xff,
                (addr >> 8) & 0xff,
        ...
        AString remoteAddrString = PlantUtils::newStringPrintf(
                "%d.%d.%d.%d",
                (addr >> 24),
                (addr >> 16) & 0xff,
                (addr >> 8) & 0xff,
...
#if 1
    int64_t nowUs = ALooper::GetNowUs();
    static int64_t kTimeIntervalUs = 10*1000000ll;
    static int64_t lastTimeUs = 0;
    
    if(lastTimeUs == 0 || (nowUs - lastTimeUs) > kTimeIntervalUs){
...
status_t ANetworkSession::start() {
    ...
    status_t err = mThread->run("ANetworkSession", ANDROID_PRIORITY_URGENT_AUDIO/*ANDROID_PRIORITY_AUDIO*/);
