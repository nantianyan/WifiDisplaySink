...
void RTPReceiver::Source::onMessageReceived(const sp<AMessage> &msg) {
    switch (msg->what()) {
        case kWhatRetransmit:
        {
            ...
            ALOGD("requesting retransmission");
        ...
        case kWhatDeclareLost:
        {
            ...
            ALOGD("Lost packet extSeqNo %d %s",
                  mAwaitingExtSeqNo,
...
void RTPReceiver::Source::onPacketReceived(
    ...
    if (mFirst) {
        ALOGV("onPacketReceived: got the first packet");
    ...
    if (udelta < kMaxDropout) {
        ...
        ALOGD("onPacketReceived: dropping the packet with a large jump SN %d. ", seq);
        return;
    } else {
...
void RTPReceiver::Source::queuePacket(const sp<ABuffer> &packet) {
    ...
    if (mFirstArrivalTimeUs < 0ll) {
        ...
        ALOGV("%s first rtp time: %lld", __FUNCTION__, mFirstRTPTimeUs);
    }
...
void RTPReceiver::Source::dequeueMore() {
    ...
    if (mPackets.empty()) {
        //ALOGV("dequeueMore: mPackets is empty");
        return;
    }
...
status_t RTPReceiver::initAsync(
    ...
    sp<AMessage> rtpNotify = PlantUtils::newAMessage(kWhatRTPNotify, this);

    sp<AMessage> rtcpNotify;
    if (rtcpMode != TRANSPORT_NONE) {
        rtcpNotify = PlantUtils::newAMessage(kWhatRTCPNotify, this);
    }
...
void RTPReceiver::scheduleSendRR() {
    (PlantUtils::newAMessage(kWhatSendRR, this))->post(5000000ll);
}
...
void RTPReceiver::Source::postRetransmitTimer(int64_t timeUs) {
    ...
    ALOGD("postRetransmitTimer delay:%lld", delayUs);
    sp<AMessage> msg = PlantUtils::newAMessage(kWhatRetransmit, this);
...
void RTPReceiver::Source::postDeclareLostTimer(int64_t timeUs) {
    ...
    int64_t delayUs = timeUs - ALooper::GetNowUs();
    sp<AMessage> msg = PlantUtils::newAMessage(kWhatDeclareLost, this);
